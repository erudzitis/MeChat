...
const ChatListBox = ({ image, username, onClick, chatPreview, borderBottom, isActive }) => {
    return (
        <>
            <div className={`w-full flex flex-row flex-shrink-0 p-5 h-5rem ${isActive ? "surface-card" : ""} cursor-pointer transition-all transition-duration-500 hover:surface-card border-round`} onClick={onClick}>
